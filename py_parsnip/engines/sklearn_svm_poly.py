"""
Sklearn engine for Support Vector Machine with polynomial kernel

Maps svm_poly to sklearn's SVR/SVC with polynomial kernel
"""
...
@register_engine("svm_poly", "sklearn")
class SklearnSVMPolyEngine(Engine):
    """
    Sklearn engine for SVM with polynomial kernel.
    ...
    def fit(self, spec: ModelSpec, molded: MoldedData, original_training_data: Optional[pd.DataFrame] = None) -> Dict[str, Any]:
        """
        Fit SVM with polynomial kernel using sklearn.
        ...
        if spec.mode == "classification":
            ...
            from sklearn.svm import SVR as SVMModel

        # Extract predictors and outcomes
        X = molded.predictors
        y = molded.outcomes

        # SVMs don't use intercepts - remove Intercept column if present
        if "Intercept" in X.columns:
            X = X.drop(columns=["Intercept"])

        # Flatten y if it's a DataFrame or 2D array
        if isinstance(y, pd.DataFrame):
            if y.shape[1] == 1:
                y = y.iloc[:, 0]
            elif spec.mode == "classification":
                # For classification with one-hot encoded outcomes,
                # find the column with 1s to get the class labels
                # Patsy creates dummy columns like "species[A]", "species[B]"
                # We need to convert back to class labels
                if all("[" in col for col in y.columns):
                    # One-hot encoded - extract class from column name
                    class_labels = [col.split("[")[1].rstrip("]") for col in y.columns]
                    y = pd.Series([class_labels[idx] for idx in y.values.argmax(axis=1)])
                else:
                    # Not one-hot encoded, just take first column
                    y = y.iloc[:, 0]
        elif isinstance(y, np.ndarray) and len(y.shape) == 2 and y.shape[1] > 1 and spec.mode == "classification":
            # Handle numpy array one-hot encoding
        ...
        if "C" not in model_args:
            model_args["C"] = 1.0
        if "degree" not in model_args:
            model_args["degree"] = 3  # Default cubic polynomial
        if "gamma" not in model_args:
            model_args["gamma"] = "scale"  # 1 / (n_features * X.var())
        if "epsilon" not in model_args and spec.mode == "regression":
            model_args["epsilon"] = 0.1

        # Force polynomial kernel
        model_args["kernel"] = "poly"

        # Create and fit model
        model = SVMModel(**model_args)
        model.fit(X, y)
        ...
        if spec.mode == "regression":
            ...
            # For classification, residuals not applicable
            residuals = None
        ...
        return {
            ...
            "n_obs": X.shape[0],
            "model_class": "SVC" if spec.mode == "classification" else "SVR",
    ...
    def predict(
        ...
        if fit.spec.mode == "regression":
            if type != "numeric":
                raise ValueError(
                    f"svm_poly with mode='regression' only supports type='numeric', got '{type}'"
                )
            ...
            if type == "class":
                ...
                raise ValueError(
                    f"svm_poly with mode='classification' supports type='class' or 'prob', got '{type}'"
                )
    ...
    def extract_outputs(
        ...
        """
        Extract comprehensive three-DataFrame output for SVM-Poly.

        Returns:
            Tuple of (outputs, coefficients, stats)

            - Outputs: Observation-level (actuals, fitted, forecast, residuals, split)
            - Coefficients: Empty DataFrame (polynomial kernel is non-parametric)
        ...
        if y_train is not None and fitted is not None:
            ...
            # Add residuals (only for regression)
            if residuals is not None:
                train_df["residuals"] = residuals
            elif fit.spec.mode == "regression":
        ...
        # ====================
        # 2. COEFFICIENTS DataFrame (Empty for polynomial SVM - it's non-parametric)
        # ====================
        # Polynomial kernel SVM is non-parametric, no meaningful coefficients
        ...
        # Only calculate metrics for regression mode
        if fit.spec.mode == "regression":
            # Training metrics
            if y_train is not None and fitted is not None:
            ...
            if "test_predictions" in fit.evaluation_data and fit.spec.mode == "regression":
                test_actuals = test_data[outcome_col].values
        ...
        stats_rows.extend([
            ...
            {"metric": "kernel", "value": model.kernel, "split": ""},
            {"metric": "C", "value": model.C, "split": ""},
            {"metric": "degree", "value": model.degree, "split": ""},
            {"metric": "gamma", "value": str(model.gamma), "split": ""},  # May be string "scale"
            {"metric": "n_support", "value": len(model.support_), "split": ""},
            {"metric": "n_features", "value": fit.fit_data["n_features"], "split": ""},
            {"metric": "n_obs_train", "value": fit.fit_data.get("n_obs", 0), "split": "train"},
        ])

        # Add epsilon for regression mode
        if fit.spec.mode == "regression":
            stats_rows.append({"metric": "epsilon", "value": model.epsilon, "split": ""})
