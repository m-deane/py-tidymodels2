"""
LightGBM engine for boosted trees

Maps boost_tree to LightGBM's LGBMRegressor:
- trees → n_estimators
...
@register_engine("boost_tree", "lightgbm")
class LightGBMBoostTreeEngine(Engine):
    """
    LightGBM engine for boosted trees.
    ...
    def fit(self, spec: ModelSpec, molded: MoldedData) -> Dict[str, Any]:
        ...
        if "sample_size" in args:
            ...
            # LightGBM requires subsample_freq when subsample < 1.0
            if args["sample_size"] < 1.0:
                model_args["subsample_freq"] = 1
        ...
        if "stop_iter" in args:
            ...
            fit_params["callbacks"] = [
                ...
                # We'll use the deprecated parameter for simplicity
            ]
            # Note: LightGBM deprecated early_stopping_rounds in fit()
            # but we can still use it via model init
            model_args["n_estimators"] = args.get("trees", 100)
            # We'll handle early stopping via callbacks if needed
        else:
            X_train, y_train = X, y
        ...
        if "stop_iter" in args:
            try:
                ...
                fit_params["callbacks"] = [early_stopping(stopping_rounds=args["stop_iter"], verbose=False)]
                model.fit(X_train, y_train, **fit_params)
            except (ImportError, TypeError):
                # Fall back to deprecated API
                model.fit(X_train, y_train, eval_set=eval_set, early_stopping_rounds=args["stop_iter"], verbose=False)
        else:
            model.fit(X_train, y_train, **fit_params)
