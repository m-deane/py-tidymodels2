"""
Sklearn engine for Support Vector Machine with RBF kernel

Maps svm_rbf to sklearn's SVR with RBF kernel
"""
...
@register_engine("svm_rbf", "sklearn")
class SklearnSVMRBFEngine(Engine):
    """
    Sklearn engine for SVM with RBF kernel.
    ...
    def fit(self, spec: ModelSpec, molded: MoldedData) -> Dict[str, Any]:
        """
        Fit SVM with RBF kernel using sklearn.
    ...
    def predict(
        ...
        if type != "numeric":
            raise ValueError(
                f"svm_rbf only supports type='numeric', got '{type}'"
            )
    ...
    def extract_outputs(
        ...
        """
        Extract comprehensive three-DataFrame output for SVM-RBF.

        Returns:
            Tuple of (outputs, coefficients, stats)

            - Outputs: Observation-level (actuals, fitted, forecast, residuals, split)
            - Coefficients: Empty DataFrame (RBF kernel is non-parametric)
        ...
        # ====================
        # 2. COEFFICIENTS DataFrame (Empty for RBF SVM - it's non-parametric)
        # ====================
        # RBF kernel SVM is non-parametric, no meaningful coefficients
        ...
        stats_rows.extend([
            ...
            {"metric": "epsilon", "value": model.epsilon, "split": ""},
