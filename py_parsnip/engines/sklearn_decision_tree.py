"""
Sklearn engine for decision tree
...
@register_engine("decision_tree", "sklearn")
class SklearnDecisionTreeEngine(Engine):
    """
    Sklearn engine for decision tree.
    ...
    def fit(self, spec: ModelSpec, molded: MoldedData, original_training_data: Optional[pd.DataFrame] = None) -> Dict[str, Any]:
        ...
        # Decision trees don't use intercepts - remove Intercept column if present
        ...
        model = DecisionTreeRegressor(**model_args)
        model.fit(X, y)
    ...
    def predict(
        ...
        if type != "numeric":
            raise ValueError(
                f"decision_tree only supports type='numeric', got '{type}'"
            )
    ...
    def extract_outputs(
        ...
        """
        Extract comprehensive three-DataFrame output for decision tree.
        ...
        # ====================
        # 2. COEFFICIENTS DataFrame (Feature Importances for Decision Tree)
        # ====================
        feature_names = fit.fit_data.get("feature_names", [])
        feature_importances = model.feature_importances_

        # For Decision Tree, we report feature importances instead of coefficients
        ...
        stats_rows.extend([
            ...
            {"metric": "max_depth", "value": model.max_depth if model.max_depth is not None else np.nan, "split": ""},
            {"metric": "n_leaves", "value": model.get_n_leaves(), "split": ""},
