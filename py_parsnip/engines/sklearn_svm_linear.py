"""
Sklearn engine for Support Vector Machine with Linear kernel

Maps svm_linear to sklearn's LinearSVR
"""
...
@register_engine("svm_linear", "sklearn")
class SklearnSVMLinearEngine(Engine):
    """
    Sklearn engine for SVM with linear kernel.
    ...
    def fit(self, spec: ModelSpec, molded: MoldedData, original_training_data: Optional[pd.DataFrame] = None) -> Dict[str, Any]:
        ...
        # LinearSVR doesn't use intercepts - remove Intercept column if present
    ...
    def predict(
        ...
        if type != "numeric":
            raise ValueError(
                f"svm_linear only supports type='numeric', got '{type}'"
            )
    ...
    def extract_outputs(
        ...
        """
        Extract comprehensive three-DataFrame output for Linear SVM.

        Returns:
            Tuple of (outputs, coefficients, stats)

            - Outputs: Observation-level (actuals, fitted, forecast, residuals, split)
            - Coefficients: Linear coefficients from the SVM
        ...
        # Linear SVM has coefficients but no statistical inference (p-values, etc.)
        ...
        stats_rows.extend([
            ...
            {"metric": "intercept", "value": model.intercept_[0] if hasattr(model, 'intercept_') else np.nan, "split": ""},
