"""
Sklearn engine for k-nearest neighbors

Maps nearest_neighbor to sklearn's KNeighborsRegressor
"""
...
@register_engine("nearest_neighbor", "sklearn")
class SklearnNearestNeighborEngine(Engine):
    """
    Sklearn engine for k-nearest neighbors.
    ...
    def fit(self, spec: ModelSpec, molded: MoldedData) -> Dict[str, Any]:
        """
        Fit k-nearest neighbors model using sklearn.
        ...
        # KNN doesn't use intercepts - remove Intercept column if present
        ...
        if "weights" not in model_args:
            model_args["weights"] = "uniform"
        if "p" not in model_args:
            model_args["p"] = 2  # Euclidean distance
    ...
    def predict(
        ...
        if type != "numeric":
            raise ValueError(
                f"nearest_neighbor only supports type='numeric', got '{type}'"
            )
    ...
    def extract_outputs(
        ...
        """
        Extract comprehensive three-DataFrame output for k-nearest neighbors.

        Returns:
            Tuple of (outputs, coefficients, stats)

            - Outputs: Observation-level (actuals, fitted, forecast, residuals, split)
            - Coefficients: Empty DataFrame (KNN is non-parametric)
        ...
        # ====================
        # 2. COEFFICIENTS DataFrame (Empty for KNN - it's non-parametric)
        # ====================
        # KNN is a non-parametric method, so no coefficients or feature importances
        ...
        stats_rows.extend([
            ...
            {"metric": "n_neighbors", "value": model.n_neighbors, "split": ""},
            {"metric": "weights", "value": model.weights, "split": ""},
            {"metric": "p", "value": model.p, "split": ""},
