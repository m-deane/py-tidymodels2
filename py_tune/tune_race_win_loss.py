"""
Efficient grid search via racing with Bradley-Terry win/loss statistics.

Racing eliminates poor parameter configurations early during cross-validation
using Bradley-Terry models to estimate "winning ability" of each configuration.
"""
...
from .racing import RaceControl, control_race, filter_parameters_bt, restore_rset, randomize_resamples
from .tune import tune_grid, TuneResults


def tune_race_win_loss(
    workflow,
    resamples,
    ...
    """
    Efficient grid search via racing with Bradley-Terry win/loss models.

    Racing methods evaluate all parameter configurations on an initial set of
    resamples (burn-in period), then use Bradley-Terry models to estimate the
    "winning ability" of each configuration and eliminate those unlikely to win.

    Algorithm:
    1. Evaluate all configs on burn_in resamples (e.g., first 3 CV folds)
    2. Compute pairwise win/loss records across resamples
    3. Fit Bradley-Terry model to estimate winning abilities
    4. Eliminate configs whose confidence interval indicates poor performance
    5. Evaluate remaining configs on next resample
    6. Repeat steps 2-5 until one config remains or all resamples used

    The Bradley-Terry model estimates the probability that configuration i
    beats configuration j as: P(i beats j) = ability_i / (ability_i + ability_j)
    ...
    Examples:
        >>> from py_tune import tune_race_win_loss, control_race
    ...
    for resample_idx in range(control.burn_in, n_resamples):
        # Filter current results with Bradley-Terry
        filter_results = filter_parameters_bt(
            results,
            alpha=control.alpha,
