"""
PyMC engine for Bayesian Poisson regression.

Implements MCMC-based Bayesian inference for Poisson GLM (count data).
"""
...
@register_engine("poisson_bayes", "pymc")
class PymcPoissonEngine(Engine):
    """
    PyMC engine for Bayesian Poisson regression.

    Models count data using Poisson likelihood with log link:
        log(λ) = α + βX
        y ~ Poisson(λ)

    where λ is the rate parameter (expected count).
    """
    ...
    def fit(
        ...
        """
        Fit Bayesian Poisson regression via PyMC.
        ...
        # Validate y is non-negative integers
        y_array = y.values if isinstance(y, pd.Series) else y
        if not np.all(y_array >= 0):
            raise ValueError("Poisson regression requires non-negative outcome values")
        if not np.all(y_array == np.floor(y_array)):
            warnings.warn("Poisson regression expects integer counts. Non-integer values will be used as-is.")
        ...
        with pm.Model() as model:
            ...
            log_lambda = intercept + pm.math.dot(X_array, beta)

            # Poisson likelihood
            y_obs = pm.Poisson("y_obs", mu=pm.math.exp(log_lambda), observed=y_array)
        ...
        # Compute fitted values (posterior mean of expected counts)
        ...
        log_lambda_mean = intercept_mean + X_array @ beta_mean
        fitted = np.exp(log_lambda_mean)  # Expected counts
    ...
    def predict(
        ...
        """
        Generate predictions from Bayesian Poisson model.

        Args:
            fit: ModelFit with fitted model
            molded: MoldedData with predictors
            type: Prediction type
                - "numeric": Posterior mean expected count
                - "conf_int": Credible intervals for expected count
                - "posterior": Posterior samples of expected count
                - "predictive": Posterior predictive samples (with Poisson noise)
        ...
        if type == "numeric":
            ...
            log_lambda = intercept + X_test_array @ beta
            pred = np.exp(log_lambda)  # Expected count
        ...
        # Credible intervals
            ...
            # Compute log(lambda) for each sample
            log_lambda_samples = intercept_samples[:, None] + beta_samples @ X_test_array.T

            # Transform to expected count
            lambda_samples = np.exp(log_lambda_samples)

            # Compute quantiles
            pred_mean = lambda_samples.mean(axis=0)
            pred_lower = np.percentile(lambda_samples, lower_q * 100, axis=0)
            pred_upper = np.percentile(lambda_samples, upper_q * 100, axis=0)
        ...
        # Posterior samples (no Poisson noise)
        elif type == "posterior":
            ...
            cols = {f".pred_sample_{i+1}": lambda_samples[i] for i in range(n_samples)}
            return pd.DataFrame(cols)

        # Posterior predictive (with Poisson noise)
            ...
            # Add Poisson noise
            np.random.seed(42)
            pred_samples = np.random.poisson(lambda_samples)
    ...
    def extract_outputs(
        ...
        """
        Extract comprehensive three-DataFrame output for Bayesian Poisson models.
        ...
        log_lambda_samples = intercept_samples[:, None] + beta_samples @ X_train.T
        lambda_samples = np.exp(log_lambda_samples)

        fitted_lower = np.percentile(lambda_samples, 2.5, axis=0)
        fitted_upper = np.percentile(lambda_samples, 97.5, axis=0)
        ...
        # For count data, R² is less meaningful, but include for consistency
